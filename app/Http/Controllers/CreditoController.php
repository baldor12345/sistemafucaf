    ...
    public function pagarcuota(Request $request){
        ...
        if($caja_id != 0){
            $error = DB::transaction(function() use($request, $caja_id){
                ...
                $transaccion->monto = round($cuota_interesMora+ $cuotainteres + $partecapital,1);
...
/*************--LISTAR DETALLE CUOTAS--********** */
    public function listardetallecuotas(Request $request){
        ...
        if (count($lista) > 0) {
            ...
            $lista = $resultado->paginate($filas);
            $request->replace(array('page' => $paginaactual));
            return view($this->folderview.'.listdetallecredito')->with(compact('lista','credito', 'paginacion', 'inicio', 'fin', 'entidad_credito','entidad_cuota', 'cabecera','titulo_pagocuota','credito_id','ruta','caja_id', 'opcion'));
        }
...
/*************--OBTENER MONTO TOTAL--************ */
    public function obtenermontototal(Request $request){
        ...
        return  array($valor_total, $parte_capital_total, $interes_total, $num_cuotas_pendientes);
    }
